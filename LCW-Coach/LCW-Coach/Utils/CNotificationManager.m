...
- (instancetype)init{
    if (self  = [super init]) {
        ...
        UITapGestureRecognizer *dismissTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapNotification)];
...
+ (void)showMessage:(NSString *)message withOptions:(NSDictionary *)options tapBlock:(void(^)())tapBlock {
    [CNotificationManager shareManager].tapBlock = tapBlock;
    [CNotificationManager setOptions:options];
    if ([[CNotificationManager shareManager] isShowing]) {
        [[CNotificationManager shareManager] reDisplayTitleLabel:message];
    }else{
...
- (void)showNotification:(NSString *)message{
    ...
    self.backgroundView.frame = CGRectMake(0, -kNOTIFICATION_VIEW_HEIGHT, self.backgroundView.frame.size.width, IOS7 ?  kNOTIFICATION_VIEW_HEIGHT : 44);
    ...
    [UIView animateWithDuration:.4 animations:^{
        self.backgroundView.frame = CGRectMake(0, IOS7?0:20, self.backgroundView.frame.size.width, IOS7?kNOTIFICATION_VIEW_HEIGHT:44);
...
- (void)dismissNotification{
    ...
    [UIView animateWithDuration:.25 animations:^{
        self.backgroundView.frame = CGRectMake(0, -self.backgroundView.height, self.backgroundView.frame.size.width, IOS7?kNOTIFICATION_VIEW_HEIGHT:44);
    } completion:^(BOOL finished) {
